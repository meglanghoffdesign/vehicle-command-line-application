...
class Truck extends Vehicle implements AbleToTow {
  // Declare properties of the Truck class ( The properties should include vin (string), color(string), make(string), model(string), year(number), weight(number), top speed (number), wheels(Wheel[]), and towing capacity(number))
  vin: string;
  color: string;
  make: string;
  model: string;
  ...
  tow(vehicle: Truck | Motorbike | Car): void {
    // Ensure the vehicle has the necessary properties (make, model, weight)
  ...
  override printDetails(): void {
    ...
    // Print details of the Truck class (VIN, make, model, year, weight, top speed, color, and towing capacity)
